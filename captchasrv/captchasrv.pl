...
use vars qw($opt_t $opt_o $opt_h $opt_p)
    ;    # For command-line options with Getopt
use Getopt::Std;
use File::Basename;
...
PrintTrace( 1,
          "Starting $Progname with options:\n"
        . "-t $TraceLvl -o "
        . ( $opt_o eq "" ? "STDOUT" : $Logfile )
        . " -p $LocalPort" );
...
sub LaunchPeriodicCleanup {
    ...
    if ( $pid = fork ) {
        ...
        PrintTrace( 2,
            "Started periodic cleanup process for temp files, PID=$pid" );
        ...
        PrintTrace( 0,
            "Cannot create cleanup child process, no more processes available"
        );
    }
    else {
        PrintTrace( 0,
            "Cannot create cleanup child process, fork error: $!" );
    }
...
# Create temp backgroung file for small rectangle cropped out of the large image
    ...
    print {$Outdesc}
        $resultstr
        ;    # Send result to open file - Note indirect notation "{$desc}"
