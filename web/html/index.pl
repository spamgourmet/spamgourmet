...
my $config = Mail::Spamgourmet::Config->new(
    configfile => '/path/to/spamgourmet.config' );
my $util = Mail::Spamgourmet::WebUtil->new( config => $config );

my $session = Mail::Spamgourmet::Session->new(
    config  => $config,
    webutil => $util,
    query   => new CGI
);
...
sub getLanguageList {
    my $session  = shift;
    my $advanced = 0;
    if ( ( $session->{'Prefix'} || $session->param('advanced') )
    ...
    $list->setTags(
        'action',             $thisscript,
        'DA',                 $selected{'DA'},
        'languageDanish',     $session->getDialog('languageDanish'),
        'DE',                 $selected{'DE'},
        'languageGerman',     $session->getDialog('languageGerman'),
        'EN',                 $selected{'EN'},
        'languageEnglish',    $session->getDialog('languageEnglish'),
        'ES',                 $selected{'ES'},
        'languageSpanish',    $session->getDialog('languageSpanish'),
        'FR',                 $selected{'FR'},
        'languageFrench',     $session->getDialog('languageFrench'),
        'IT',                 $selected{'IT'},
        'languageItalian',    $session->getDialog('languageItalian'),
        'JA',                 $selected{'JA'},
        'languageJapanese',   $session->getDialog('languageJapanese'),
        'KO',                 $selected{'KO'},
        'languageKorean',     $session->getDialog('languageKorean'),
        'NL',                 $selected{'NL'},
        'languageDutch',      $session->getDialog('languageDutch'),
        'NO',                 $selected{'NO'},
        'languageNorwegian',  $session->getDialog('languageNorwegian'),
        'PL',                 $selected{'PL'},
        'languagePolish',     $session->getDialog('languagePolish'),
        'PT',                 $selected{'PT'},
        'languagePortuguese', $session->getDialog('languagePortuguese'),
        'RO',                 $selected{'RO'},
        'languageRomanian',   $session->getDialog('languageRomanian'),
        'RU',                 $selected{'RU'},
        'languageRussian',    $session->getDialog('languageRussian'),
        'SV',                 $selected{'SV'},
        'languageSwedish',    $session->getDialog('languageSwedish'),
        'TR',                 $selected{'TR'},
        'languageTurkish',    $session->getDialog('languageTurkish'),
        'ZH',                 $selected{'ZH'},
        'languageChinese',    $session->getDialog('languageChinese')
    );
...
sub getJS {
    my $session = shift;
    my $page    = $pagemaker->new(
        template     => 'stuff.js',
        languageCode => $session->getLanguageCode()
    );
    $page->setTags(
        'nodelimitersinprefix',
        $session->getDialog( 'badcharacterinprefix', 'character', '.' ),
        'enterusernamepassword',
        $session->getDialog('enterusernamepassword'),
        'provideusername',
        $session->getDialog('provideusername'),
        'nodelimitersinusername',
        $session->getDialog(
            'badcharacterinusername', 'character',
            '.',                      'username',
            '\'+obj.newuser.value+\''
        ),
        'nospacesinusername',
        ...
        'providepassword',
        $session->getDialog('providepassword'),
        'provideforwardingaddress',
        $session->getDialog('provideforwardingaddress'),
        'entercurrentpassword',
        $session->getDialog('entercurrentpassword'),
        'passwordmismatch',
        $session->getDialog('passwordmismatch'),
        'invalidforwardingaddress',
        $session->getDialog('invalidforwardingaddress')
    );
    return $page;
}
...
sub printPage {
    ...
    if ( $input eq 'links.html' ) {
        $page->printPage(
            'partnersprizes', $session->getDialog('partnersprizes'),
        ...
        $page->printPage(
            'linktospamgourmet', $session->getDialog('linktospamgourmet'),
            'howtolink',         $session->getDialog('howtolink')
        );
    }
    elsif ( $input eq 'donate.html' ) {
        my $comments = $pagemaker->new( template => 'donationcomments.html' )
            ->getContent();
        $page->printPage(
            'donatepaypal',     $session->getDialog('donatepaypal'),
...
sub confirmemailchange {
    ...
    my $session = shift;
    my $hash
        = $session->param('confirmemailchange') || $session->param('cec');
    ...
    if ($hash) {
     ...
     FROM Users WHERE PendingHashCode = ?";
        ...
        if ( $UserID && $realaddress ne $address ) {
            $sql
                = "UPDATE Users SET RealEmail = PendingEmail WHERE UserID = ?";
            $st = $config->db->prepare($sql);
            $st->execute($UserID);
            $msg = $session->getDialog( 'confirmationsuccessful', 'address',
                $address );
            $session->{'RealEmail'} = $address;
            my $now = time();
            $sql
                = "INSERT INTO AddressAudit (UserID,OldAddress,NewAddress,ChangeTime,IPAddress) VALUES (?,?,?,?,?);";
            $st = $config->db->prepare($sql);
            $st->execute( $UserID, $realaddress, $address, $now,
                $ENV{'REMOTE_ADDR'} );

########### send changed email message to old address
            my $wm = Mail::Spamgourmet::WebMessages->new( config => $config );
            $wm->sendchangedemailmessage( $session, $thisscript, $realaddress,
                $address );

        }
        elsif ($UserID) {
            $msg = $session->getDialog( 'confirmedalready', 'address',
                $address );
        }
    }
    if ( !$UserID ) {
...
sub printMyXml {
    ...
    my ($EmailID,    $Word,         $MaxCount,  $Count,
        $NumDeleted, $NumForwarded, $TimeAdded, $Sender,
        $Address,    $Hidden,       $Note
    ) = ( 0, '', 0, 0, 0, 0, 0, '', '', 0, '' );
    ...
    $sql
        = "SELECT EmailID, Word, InitialCount, Count, NumDeleted, NumForwarded,
   ...
   FROM Emails WHERE UserID = ?";
    ...
    $st->execute($UserID);
    $st->bind_columns(
        \%attr,   \$EmailID,    \$Word,         \$MaxCount,
        \$Count,  \$NumDeleted, \$NumForwarded, \$TimeAdded,
        \$Sender, \$Address,    \$Hidden,       \$Note
    );
    ...
    while ( $st->fetch() ) {
        ...
        $row->setTags(
            'ID',         $EmailID,    'word',         $Word,
            'maxcount',   $MaxCount,   'count',        $Count,
            'numdeleted', $NumDeleted, 'numforwarded', $NumForwarded,
            'timeadded',  $TimeAdded,  'address',      $Address,
            'sender',     $Sender,     'hidden',       $Hidden,
            'notes',      $Note
        );
        $page->concatenate($row);
    }
    $bottom = $pagemaker->new(
        'config'     => $config,
        languageCode => $session->getLanguageCode()
    );
...
sub getSignUpForm {
    ...
    my $session = shift;
    my $util    = shift;
    my $signupformtemplate
        = $config->getCaptchagenHost()
        ? 'signupform.html'
        : 'signupformnocaptcha.html';
    my $page = $pagemaker->new(
        config       => $config,
        languageCode => $session->getLanguageCode(),
    ...
    $page->setTags(
        'imagefilename',
        $session->getImageFileName(),
        'loginmsg',
        $session->{'loginmsg'},
        'action',
        $thisscript,
        'newuser',
        $util->webSanitize( $session->param('newuser') ),
        'imagehash',
        $session->getImageHash(),
        'typeimageword',
        $session->getDialog('typeimageword'),
        'providepassword',
        $session->getDialog('providepassword'),
        'newpass',
        $session->getDialog('newpass'),
        'confirm',
        $session->getDialog('confirm'),
        'realemail',
        $util->webSanitize( $session->param('realemail') ),
        'go',
        $session->getDialog('go'),
        'cannotreadimage',
        $session->getDialog('cannotreadimage')
    );
    $session->{'loginmsg'} = ''
        ; # we've moved this to the form, so we don't want it to show on the main page
    return $page;
}
...
sub getMySendToForm {
    ...
    my $disposable = shift;
    my $msg        = shift;
    my ( $sql, $st, %attr, $EmailID, $Address, $page )
        = ( '', 0, {}, 0, '', '' );
   ...
   FROM Emails WHERE UserID = ? AND ((Count > -10 AND Hidden <> 1) OR EmailID = ?  );";
    ...
    while ( $st->fetch() ) {
        ...
        $option = $pagemaker->new(
            template     => 'sendfromoption',
            languageCode => $session->getLanguageCode()
        );
        $option->setTags(
            'emailid', $EmailID, 'selected', $selected,
            'address', $Address
        );
...
sub getMyUpdateForm {
    ...
    my ($EmailID,   $Word,         $MaxCount, $Count,
        $TimeAdded, $Sender,       $Address,  $Hidden,
        $Note,      $NumForwarded, $NumDeleted
    );
    ...
    $sql
        = "SELECT EmailID, Word, InitialCount, Count, TimeAdded, Sender, Address, Hidden, Note,
   ...
   FROM Emails WHERE UserID = ? AND EmailID = ?;";
    ...
    $st->bind_columns(
        \%attr,   \$EmailID,   \$Word,         \$MaxCount,
        \$Count,  \$TimeAdded, \$Sender,       \$Address,
        \$Hidden, \$Note,      \$NumForwarded, \$NumDeleted
    );
    $st->fetch();
    $page = $pagemaker->new(
        config       => $config,
    ...
    if ( $Count >= -10 ) {
        ...
        $Address = $Word
            if !$Address;    # before Feb, 2001, Addresses weren't stored...
        my $message = $Address;
        ...
        $page->setTags(
            'message', $message, 'action', $thisscript, 'emailid', $EmailID,
...
sub getEditExemplarForm {
    ...
    my $disposable = shift;
    my $msg        = shift;
    my ( $sql, $st, %attr, $EmailID, $Address, $Word, $Number, $page )
        = ( '', 0, {}, 0, '', '', '', '' );
    $disposable = 0 if !$disposable;
   ...
   FROM Emails WHERE UserID = ? AND EmailID = ?";
    ...
    $page = $pagemaker->new(
        template     => 'editexemplarform.html',
        languageCode => $session->getLanguageCode()
    );
    my $prefix      = '';
    my $prefixinput = '';
    ...
    if ( $prefix || $session->Prefix ) {
        $prefixinput = $pagemaker->new(
            template     => 'prefixtextinput',
            languageCode => $session->getLanguageCode()
        );
        $prefixinput->setTags( 'prefix', $session->getDialog('prefix'),
            'theprefix', $prefix );
    }
    else {
        $prefixinput = $pagemaker->new(
            template     => 'prefixhiddeninput',
            languageCode => $session->getLanguageCode()
        );
    }
    ...
    $page->setTags(
        ...
        $msg,
        'domainrequired',
        $session->getDialog('domainrequired'),
        'domainnotinlist',
        $session->getDialog('domainnotinlist'),
        'canonlychangecase',
        $session->getDialog('canonlychangecase'),
        'returntoadvancedmode',
        $session->getDialog('returntoadvancedmode'),
        'disposable',
        $EmailID,
        'prefixinput',
        $prefixinput->getContent(),
        'currentprefix',
        $session->Prefix(),
        'prefixmismatch',
        $session->getDialog('prefixmismatch'),
        'numberrequiredforprefix',
        $session->getDialog('numberrequiredforprefix'),
        'word',
        $session->getDialog('word'),
        'theword',
        $mword,
        'username',
        $musername,
        ...
        $session->getUserName(),
        'user',
        $session->getDialog('user'),
        'number',
        $session->getDialog('number'),
        'thenumber',
        $mcount,
        'thedomain',
        $selecteddomain,
        ...
        'go',
        $session->getDialog('go'),
        'editexemplardescription',
        $session->getDialog('editexemplardescription')
    );
    return $page;
}
...
sub myemails {
    ...
    if ($UserID) {
        ...
        if ( $session->param('myupdateform') ) {
            $page = &getMyUpdateForm( $config, $session,
                $session->param('disposable') );
        }
        elsif ( $session->param('mysendtoform') ) {
            $page = &getMySendToForm( $config, $session,
                $session->param('disposable') );
         ...
         FROM Emails WHERE EmailID = ? AND UserID = ? AND Count > -10;';
                ...
                $st->fetch();
                if ($Word)
                {    # if it's there and not disabled, refresh the address
                    $sql
                        = 'UPDATE Emails SET Count = InitialCount, Hidden = 0 WHERE
                ...
                $sql
                    = "SELECT EmailID FROM Emails WHERE Word = ? AND UserID = ?";
                $st = $config->db->prepare($sql);
                ...
                if ( $st->fetch() ) {
                    ...
                    $st->fetch();
                    my $count = $util->getNumberFromString( $number,
                        $defaultnumber );
           ...
           VALUES (?,?,?,?,?,?,?,?);";
            ...
            if ($alreadyexists) {
                my $msg = $session->getDialog( 'addressalreadyexists', 'word',
                    $Word );
                $page = &getMySendToForm( $config, $session, $disposable,
                    $msg );
            }
            else {
                if ($Word) {
                    $toAddress
                        = $util->getRedirectedAddress( $recipient, $Word,
                        $session->getUserName(), $ph );
                }
                if ( !$session->hasFeature('MASKFORWARD') )
                {    # turn on address masking if it's not on
                    $session->addFeature(
                        $config->getFeature('MASKFORWARD') );
                ...
                $page->setTags(
                    ...
                    $session->getDialog('returntoadvancedmode'),
                    'usethisaddress',
                    $session->getDialog('usethisaddress'),
                    'messagewillbefrom',
                    $session->getDialog('messagewillbefrom'),
                    'recipient',
                    $recipient,
                    'action',
                    $thisscript,
            ...
            $page = &getEditExemplarForm( $config, $session,
                $session->param('disposable') );
        }
        else {
            ...
            if ( $session->param('editexemplar') ) {
                ...
                my $ExistingWord;
                $sql
                    = "SELECT Word FROM Emails WHERE EmailID = ? AND UserID = ?";
                $st = $config->db->prepare($sql);
                ...
                $sql
                    = "UPDATE Emails SET Address = ? WHERE EmailID = ? AND UserID = ?";
                $st = $config->db->prepare($sql);
            ...
            if ( $session->param('deleteaddress') ) {
                ...
                $sql
                    = "SELECT Address FROM Emails WHERE EmailID = ? AND UserID = ?";
                my $adr;
                $st = $config->db->prepare($sql);
                $st->execute( $EmailID, $UserID );
            ...
            if ( $session->param('updateemail') ) {
                ...
                $sql
                    = "UPDATE Emails SET InitialCount = ?, Count = ?, Sender = ?, Hidden = ?, Note = ?
         WHERE EmailID = ? AND UserID = ? AND Count >= -10";
                $st = $config->db->prepare($sql);
                $st->execute(
                    $Count, $Count,   $Sender, $Hidden,
                    $Note,  $EmailID, $UserID
                );
                $showAddressInSearch = $EmailID;
            }
            ...
            if ($showAddressInSearch) {
                ...
                $searchrestriction
                    = $util->getSearchRestriction( $searchterms, 1, 'Prefix',
                    'Word', 'Address', 'Sender', 'Note' );
            }
            srand();
            my $r = rand(10000);
            ...
            $hiddenrestriction = ' AND HIDDEN = 0 '
                if !$showHidden && !$showAddressInSearch;
       ...
       ORDER BY $orderby";
            ...
            while ( $st->fetch() ) {
                ...
                $Address
                    = $util->highlight( $searchterms, $Address, '#FFFF00' );
                $Sender
                    = $util->highlight( $searchterms, $Sender, '#FFFF00' );
                ...
                $row->setTags(
                    ...
                    'searchterms',  $urlsearchterms,
                    'maxcount',     $MaxCount,
                    'count',        $Count,
                    'numdeleted',   $NumDeleted,
                    'numforwarded', $NumForwarded,
                    'timeadded',    $TimeAdded,
                    'address',      $Address,
                    'sender',       $Sender,
                    'hidden',       $Hidden,
...
#      }
            ...
            $page->setTags(
                'yourdisposableaddresses',
                $session->getDialog('yourdisposableaddresses'),
                'search',
                $session->getDialog('search'),
                'showhiddenaddresses',
                $session->getDialog('showhiddenaddresses'),
                'word',
                $session->getDialog('word'),
                'clicktoedit',
                $session->getDialog('clicktoedit'),
                'maxcount',
                $session->getDialog('maxcount'),
                'remaining',
                $session->getDialog('remaining'),
                'forwarded',
                $session->getDialog('forwarded'),
                'deleted',
                $session->getDialog('deleted'),
                'created',
                $session->getDialog('created'),
                'fulladdress',
                $session->getDialog('fulladdress'),
                'exclusivesender',
                $session->getDialog('exclusivesender'),
                'note',
                $session->getDialog('note')
            );
...
sub getYourStats {
    ...
    if ( $session->{'UserID'} ) {
        ...
        $yourstats
            = $session->getDialog( 'yourstats', 'forwarded', $f, 'eaten',
            $d );
        ...
        $st->fetch();
        $yourstats .= $session->getDialog( 'youraddresscount', 'addresscount',
            $useraddresscount );
    }
...
sub mainPage {
    ...
    my $session = shift;
    my $util    = shift;
    my ($msg,           $pendingmsg,    $sendercount,    $watchwordcount,
        $serverstats,   $maskforwardon, $maskforwardoff, $watchwordson,
        $watchwordsoff, $emlon,         $emloff,         $dlhmon,
        $dlhmoff,       $hston,         $hstoff,         $tsoon,
        $tsooff,        $todaycount,    $counterID,      $weekchart,
        $normallink,    $securelink
    );
    ...
    if ( &secureMode() ) {
        $normallink
            = '<a href="'
            . $config->getConfigParameter('normalURL')
            . $thisscript
            . '?languageCode='
            . $session->getLanguageCode() . '">'
            . $session->getDialog('normal') . '</a>';
        ...
        $securelink
            = '<a href="'
            . $config->getConfigParameter('secureURL')
            . $thisscript
            . '?languageCode='
            . $session->getLanguageCode() . '">'
            . $session->getDialog('secure') . '</a>';
    }
...
## get more current info here
        ...
        while ( $st->fetch() ) {
            ...
            $Senders
                .= "<br>$Sender (<a href=\"$thisscript?deletesender=$TrustedID&advanced=1\">"
                . $session->getDialog('delete')
                . "</a>)\n";
            $sendercount++;
        }
        if ($Senders) {
            $Senders
                = $session->getDialog( 'yourtrustedsenders', 'sendercount',
                $sendercount )
                . $Senders;
        }
        ...
        $sql
            = "SELECT WatchwordID, Watchword FROM Watchwords WHERE UserID = ?";
        $st = $config->db->prepare($sql);
        $st->execute($UserID);
        ...
        while ( $st->fetch() ) {
            ...
            $Watchwords
                .= "<br>$Watchword (<a href=\"$thisscript?deletewatchword=$WatchwordID&advanced=1\">"
                . $session->getDialog('delete')
                . "</a>)\n";
            $watchwordcount++;
        }
        if ($Watchwords) {
            $Watchwords
                = $session->getDialog( 'yourwatchwords', 'watchwordcount',
                $watchwordcount )
                . $Watchwords;
        }
    }
    $Senders = $session->getDialog( 'yourtrustedsenders', 'sendercount', '0' )
        if !$Senders;
    $Watchwords
        = $session->getDialog( 'yourwatchwords', 'watchwordcount', '0' )
        if !$Watchwords;
...
## set pending msg, if needed
    if (!$session->RealEmail() && $session->PendingEmail()
        || (   $session->PendingEmail()
            && $session->RealEmail() ne $session->PendingEmail() )
        )
    {    # && !param('realemail')) {
        $pendingmsg = $session->getDialog( 'pendingmessage', 'paddr',
            $session->PendingEmail() );
    }

## gluttony counts

    ( $serverstats, $counterID, $todaycount )
        = &getServerStats( $config, $session );

    $weekchart = &getWeekchart( $session, $counterID, $todaycount )
        if $counterID;

    $st = '';
    my $mainpage = $pagemaker->new(
        template     => 'mainpage.html',
        languageCode => $session->getLanguageCode()
    );
    my $flags = $pagemaker->new(
        template     => 'flags.html',
        languageCode => $session->getLanguageCode()
    );
    ...
    my $adsensemedrect = '';
    my $explorerkiller
        = '';   #$pagemaker->new(template=>'explorernotkiller')->getContent();
    my $analytics    = '';
    my $adsensesmall = '';

    if ( !&secureMode() ) {
        $adsense = $pagemaker->new(
            template     => 'adsense',
            languageCode => $session->getLanguageCode()
        );
        $adsense->setTags( 'addisclaimer',
            $session->getDialog('addisclaimer') );
...
#    $adsensemedrect = $pagemaker->new(template=>'adsensemedrect',languageCode=>$session->getLanguageCode())->getContent();
    ...
    my $loginform
        = $pagemaker->new( languageCode => $session->getLanguageCode() );
    ...
    if ( !$UserID ) {
        ...
        my $remembermecheck = '';
        my $rememberme      = '';
        $remembermecheck = $pagemaker->new(
            template     => 'remembermecheck',
            languageCode => $session->getLanguageCode()
        );
        $rememberme = $session->getDialog('rememberme');
        $loginform->setTags(
            'action',
            $config->getConfigParameter('secureURL') . $thisscript,
            'login',
            $session->getDialog('login'),
            'user',
            $session->getDialog('user'),
            'pass',
            $session->getDialog('pass'),
            'remembermecheck',
            $remembermecheck->getContent(),
            'rememberme',
            $rememberme,
            'forgotpassword',
            $session->getDialog('forgotpassword'),
            'createaccount',
            $session->getDialog('createaccount'),
            'newuser',
            $session->getDialog('newuser'),
            'newpass',
            $session->getDialog('newpass'),
            'confirm',
            $session->getDialog('confirm'),
            'email',
            $session->getDialog('email'),
            'go',
            $session->getDialog('go'),
            'realemail',
            $util->webSanitize( $session->param('realemail') )
        ...
        $button       = 'button';
        $onload       = 'if(!hastyped){document.loginform.user.focus();}'
            if !$session->getImageHash();
    }
    else {
        $loginform->setTemplate('loggedinform.html');
        $loginform->setTags(
            'changepassword', $session->getDialog('changepassword'),
            'currpass',       $session->getDialog('currpass'),
            'newpass',        $session->getDialog('newpass'),
            'confirm',        $session->getDialog('confirm'),
            'go',             $session->getDialog('go'),
            'logout',         $session->getDialog('logout')
        );
    ...
    if ( $session->param('myemails') ) {
        ...
        $content->setTags(
            'setnewpassword', $session->getDialog('setnewpassword'),
            'newpass',        $session->getDialog('newpass'),
            'confirm',        $session->getDialog('confirm'),
            'hc',             $session->param('hc')
        );
        ...
        $content->setTags(
            'sendpasswordreset', $session->getDialog('sendpasswordreset'),
 ...
 # $form->setTags('forwardingaddress',$session->getDialog('forwardinaddress'),
        ...
        if ( $UserID && $session->hasFeature('EATENMESSAGELOG') ) {
            ...
            if ($emlData) {
                ...
                while ($emlData) {
                    ...
                    $emlRow   = $pagemaker->new(
                        template     => $template,
                        languageCode => $session->getLanguageCode()
                    );
                    $emlRow->setTags(
                        'action',  $thisscript, 'date', $date,
                        'from',    $from,       'for',  $for,
                        'EmailID', $eid
                    );
                    $emlRows .= $emlRow->getContent();
                    $emlData = substr( $emlData, $chunksize,
                        ( length($emlData) - $chunksize ) );
                    $num++;
                }
                $stcheck = 0;
                ...
                $eml->setTags(
                    'eatenmessageinfo',
                    $session->getDialog( 'eatenmessageinfo', 'number', $num ),
                    'date',
                    $session->getDialog('date'),
                    'from',
                    $session->getDialog('from'),
                    'to',
                    $session->getDialog('to'),
                    'rows',
                    $emlRows
                );
            }
        ...
        $form->setTags(
            ...
            $Watchwords,
            'showaddresses',
            $session->getDialog('showaddresses'),
            'showhiddenaddresses',
            $session->getDialog('showhiddenaddresses'),
            'search',
            $session->getDialog('search'),
            'sendfromanaddress',
            $session->getDialog('sendfromanaddress'),
            'defaultnumber',
            $session->getDialog('defaultnumber'),
            'mydefaultnumber',
            $session->{'DefaultNumber'},
            'addtrustedsender',
            $session->getDialog('addtrustedsender'),
            'addwatchword',
            $session->getDialog('addwatchword'),
            'enabled',
            $session->getDialog('enabled'),
            'disabled',
            $session->getDialog('disabled'),
            'replyaddressmasking',
            $session->getDialog('replyaddressmasking'),
            'maskforwardon',
            $maskforwardon,
            'maskforwardoff',
            $maskforwardoff,
            'watchwordenforcement',
            $session->getDialog('watchwordenforcement'),
            'watchwordson',
            $watchwordson,
            'watchwordsoff',
            $watchwordsoff,
            'eatenmessagelog',
            $session->getDialog('eatenmessagelog'),
            'emlon',
            $emlon,
            'emloff',
            $emloff,
            'hidesubjecttagline',
            $session->getDialog('hidesubjecttagline'),
            'hidefortsonly',
            $session->getDialog('hidefortsonly'),
            'hston',
            $hston,
            'hstoff',
            $hstoff,
            ...
            'tsooff',
            $tsooff,
            'dontloghiddenaddresses',
            $session->getDialog('dontloghiddenaddresses'),
            'dlhmon',
            $dlhmon,
            'dlhmoff',
            $dlhmoff,
        ...
        $content->setTags( 'prefix', $session->Prefix, 'xmllink', $xmllink );
        $tabs->setTemplate('advancedtabs.html');
    }
...
## yourstats:
    ...
    $mainpage->printPage(
        'action', $thisscript, 'quote', $saying, 'serverstats', $serverstats,
        'motd',         $motd,                      'imageshost', $imageshost,
        'languagelist', &getLanguageList($session), 'onload',     $onload,
        'normallink',   $normallink,                'securelink', $securelink,
        'loginmsg',     $session->{'loginmsg'},

        #   'linktous',$session->getDialog('linktous'),
        'privacy',         $session->getDialog('privacy'),
        'termsconditions', $session->getDialog('termsconditions'),
        'message', $msg, 'pendingmsg', $pendingmsg, 'yourstats', $yourstats,
        'button',  $button,
        'onclick', $alert, 'forwardingaddress',
        $session->getDialog('forwardingaddress'), 'realemail',
...
sub doUpdates {
    ...
    if ( $session->param('resetpassworduser') ) {
        ...
        my %attr;
        $sql
            = "SELECT UserID, RealEmail FROM Users WHERE UserName = ? and RealEmail = ?";
        $st = $config->db->prepare($sql);
        $st->execute( $u, $e );
        ...
        if ( $st->fetch() ) {
            if ($re) {
                ...
                $sql
                    = "UPDATE Users SET PendingHashCode = ? WHERE UserID = ?";
                $st = $config->db->prepare($sql);
                $st->execute( $hashcode, $uid );
                my $wm = Mail::Spamgourmet::WebMessages->new(
                    config => $config );
                $session->setUserName($u)
                    ;    # this is so "from" email will be correct
                $wm->sendpasswordresetmessage( $session, $thisscript, $re,
                    $hashcode );
    ...
    if ( $session->getUserID() ) {
        my $UserID = $session->getUserID();
        if (   $session->param('saverealemail')
            && $session->param('realemail') ne $session->{'RealEmail'} )
        {
            my $e = $session->param('realemail');
            if (!$e
                || (  !$util->isin( $e, $config->getLocalDomains() )
                    && $util->looksRight($e) )
                )
            {
                my $now      = time();
                ...
                if ($e) {
                    $sql
                        = "UPDATE Users SET PendingEmail = ?, PendingHashCode = ? WHERE UserID = ?;";
                    $st = $config->db->prepare($sql);
                    ...
                    $session->setUserName($username)
                        ;    #for the case where it's not set
                    my $adminemail = $wm->sendconfirmationmessage( $session,
                        $thisscript, $e, $hashcode );
                ...
                $msg .= $session->getDialog( 'invalidforwardingaddress',
                    'address', $e );
            }
            else {
            ...
            if ($phc) {
                my $wm = Mail::Spamgourmet::WebMessages->new(
                    config => $config );
                my $adminemail
                    = $wm->sendconfirmationmessage( $session, $thisscript,
                    $session->PendingEmail, $phc );
                $msg = $session->getDialog( 'confirmationsent', 'adminemail',
                    $adminemail );
            }
        }
        ...
        if ( $session->param('deletesender') ) {
            $sql
                = "DELETE FROM Permitted WHERE PermittedID = ? AND UserID = ?;";
            $st = $config->db->prepare($sql);
        ...
        if ( $session->param('deletewatchword') ) {
            $sql
                = "DELETE FROM Watchwords WHERE WatchwordID = ? AND UserID = ?";
            $st = $config->db->prepare($sql);
...
### need special logic for tagline for trusted/excl, because it's currently dependent on the main hide feature
            if ( $session->param('addFeature')
                == $config->getFeature('DISABLETAGLINETRUSTEDEXCLUSIVE') )
            {
...
### need special logic for tagline for trusted/excl, because it's currently dependent on the main hide feature
            if ( $session->param('clearFeature')
                == $config->getFeature('DISABLETAGLINE') )
            {
                $session->clearFeature(
                    $config->getFeature('DISABLETAGLINETRUSTEDEXCLUSIVE') );
            }
            $sql = "UPDATE Users SET Features = ? WHERE UserID = ?;";
            $st  = $config->db->prepare($sql);
        ...
        if ( $session->{'UserID'} && !$session->{'RealEmail'} )
        {    # && !$session->param('realemail')) {
...
sub getServerStats {
    ...
    if ( -e $filename ) {
        ...
        if ( !$config->db
            || time() - $mtime < $config->localfilecachetimeout() )
        {
            open( FILE, "<$filename" );
            my $line;
    ...
    if ( !$gotfromfile ) {
        ...
        $sql
            = "SELECT NumForwarded AS ForwardCount FROM Counter WHERE CountDate = '0000-00-00';";
        $st = $config->db->prepare($sql);
        $st->execute();
        ...
        $sql
            = "SELECT NumDeleted FROM Counter WHERE CountDate = '0000-00-00';";
        $st = $config->db->prepare($sql);
        $st->execute();
        ...
        my ($undef, $undef1, $undef2, $mday, $mon,
            $year,  $undef3, $undef4, $undef5
        ) = localtime(time);
        $mon++;
        $year = int($year);
        $year += 1900 if $year < 1900;
        $sql
            = "SELECT CounterID, NumDeleted, NumForwarded FROM Counter WHERE CountDate = '$year-$mon-$mday';";
        $st = $config->db->prepare($sql);
        $st->execute();
        $st->bind_columns( \%attr, \$stats{'counterID'},
            \$stats{'todaycount'}, \$stats{'todayforwarded'} );
        $st->fetch();
    ...
    $stats = $session->getDialog(
        'stats',                  'daycount',
        $stats{'daycount'},       'usercount',
        $stats{'usercount'},      'addresscount',
        $stats{'addresscount'},   'forwardcount',
        $stats{'forwardcount'},   'todayforwarded',
        $stats{'todayforwarded'}, 'spamcount',
        $stats{'spamcount'},      'todaycount',
        $stats{'todaycount'}
    );
...
sub getWeekchart {
    if ( defined( $ENV{'HTTP_USER_AGENT'} )
        && $ENV{'HTTP_USER_AGENT'} =~ /MSIE\s5\...\;\sMac\_PowerPC/ )
    {
        return
            '<img src="stuff/graphdisabled.png" border=0 width=300 height=100 alt="">';
    }
    elsif ( &secureMode() ) {
        return
            '<img src="stuff/graphdisabledforsecuremode.png" border=0 width=300 height=100 alt="graph disabled for secure mode">';
    }
    ...
    if ( !$gotfromfile && $config->db ) {
        ...
        $sql
            = "SELECT CountDate, NumDeleted FROM Counter WHERE CounterID > ($counterID-7) AND CounterID < $counterID;";
        $st = $config->db->prepare($sql);
        $st->execute();
        $st->bind_columns( \%attr, \$countdate, \$countnum );
        $weekchart = $config->getConfigParameter('chartserver')
            . "?width=300&amp;height=100&amp;type=bars3d&amp;bar_depth=10&amp;shading=3&amp;dclrs=[pink]&amp;x_labels=[";
        ...
        $weekchart
            ...
            . $todaycount
            . ']&amp;trailing';
        my $alt = 'spam stopped this week'
            ;    # $session->getDialog('spameatenthisweek');
        $weekchart
            = "<img src=\"$weekchart\" border=0 width=300 height=100 alt=\"$alt\">";
        open( FILE, ">$filename" );
