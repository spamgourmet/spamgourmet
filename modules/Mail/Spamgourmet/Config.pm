...
use strict;
use vars
    qw{%localdomains $websessiontimeout $dbstring $dbuser $dbpassword $webapproot
...
$FEATURES{'PRESERVEREDIRECTDISPLAYNAME'}    = 29;
...
sub new {
    ...
    $self->{'websessiontimeout'}
        = $websessiontimeout;                 # default timeout in seconds
    ...
    $self->{'localfilecachetimeout'}
        = $localfilecachetimeout;    # seconds to time out local file caches
...
sub getAdminEmail {
    ...
    $self->debug(
        "username: $username; adminaccount: $adminaccount; admindomain: $admindomain; ret: $ret"
    );
    return $ret;
}
...
#  }
    ...
    while ( !$connected && $connectTries < 50 ) {
        $connected = $self->{'db'}
            = DBI->connect( $self->{'dbstring'}, $self->{'dbuser'},
            $self->{'dbpassword'} );
        if ( !$connected ) {
            $self->debug(
                "failed attempt to connect - try number: $connectTries, total time: $totaltime"
            );
            sleep $interval;
            $totaltime += $interval;
        }
    ...
    if ( !$connected ) {
        ...
        die EX_TEMPFAIL
            if $self->{'mode'} == 0
            ;   # if we're running under sendmail, we want to issue a TEMPFAIL
    }
}
...
sub debugstderr {
    my $self = shift;
    if (@_) {
        $self->{debugstderr} = shift;
    }
    return $self->{debugstderr};
}
...
sub formatNumDate {
    ...
    my $dateOnly = shift;
    if ($DateTime) {
        my ( $sec, $min, $hour, $Mdays, $Mons, $Years, $wday, $yday, $isdst )
            = ( 0, 0, 0, 0, 0, 0, 0, 0, 0 );
        ( $sec, $min, $hour, $Mdays, $Mons, $Years, $wday, $yday, $isdst )
            = localtime($DateTime);
        $Years = $Years + 1900;
        $Mons  = int( $Mons + 1 );
