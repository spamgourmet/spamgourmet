...
sub new {
    ...
    if ( $params{'config'} ) {
        ...
        die
            'WebUtil must be initialized with an instance of Mail::Spamgourmet::Config.pm';
    }
...
sub getSearchRestriction {
    ...
    ($searchterms)
        = &sanitize($searchterms)
        ;            # call the 'sanitize' function to escape chars, etc.
    ...
    {    # if the dbms supports field squishing, it's much easier and better.
        my $searchfields;
        foreach my $i ( 0 .. $#args ) {
...
sub highlight {
    ...
    if ($searchterms) {
        ...
        foreach $i ( 0 .. $#terms ) {
            $instr
                =~ s/($terms[$i])/<b style=\"background:$color\">$1<\/b>/gi;
        }
    }
    return $instr;
}
