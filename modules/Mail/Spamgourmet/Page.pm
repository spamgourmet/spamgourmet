...
my %_templatedates
    ; # hash to track template dates (and refresh the cache if they've been updated)
...
sub fetchpage {
    ...
    if ($template) {
        ...
        if ( $self->{'config'}->{'webtemplatedir'} ) {
            $templatefile
                = $self->{'config'}->{'webtemplatedir'}
                . $self->{'languageCode'} . '/'
                . $template;
        }

        my $filedate = -M $templatefile
            ;    # get template file date for comparison with cache
        ...
        if (  !$_templates{$templatefile}
            || $_templatedates{$templatefile} ne $filedate )
        {
            if ( !-e $templatefile ) {
                $templatefile
                    = $self->{'config'}->{'webtemplatedir'}
            ...
            if ( !-e $templatefile ) {
                $self->{'error'}
                    .= "Could not open templatefile: $templatefile";
                return $self;
            }
            else {
                open( MYFILE, "$templatefile" );
                sysread( MYFILE, $self->{'content'}, 100000 )
                    ;    # read in the file (100k max)
                close MYFILE;
                $_templatedates{$templatefile}
                    = $filedate;    # set the cache date to match the file
                $_templates{$templatefile} = $self->{'content'}
                    ;    # set the cache content to match the file content
            }
...
sub setTags {
    ...
    foreach $i ( 0 .. $#tag ) {
        $self->{'content'} =~ s/<%$tag[$i]%>/$string[$i]/gi
            if defined( $tag[$i] );
    }
    return $self;
}
