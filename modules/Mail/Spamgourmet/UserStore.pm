...
sub setup_test_user {
    ...
    my $sql
        = "INSERT INTO Users (UserName,RealEmail,Password,TimeAdded,IPAddress,LanguageCode,SessionToken,LastCommand)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?);";
    my $st = $self->{'config'}->db->prepare($sql);
    $st->execute( "test", 'testprotected@example.org', "dud-password", $now,
        0, $activeLocale, "junktoken", $now );
...
sub create_email_address {
    ...
    my $sql = "INSERT INTO Emails
       ...
       VALUES (?,?,?,?,?,?,?,?);";
    my $st = $self->{'config'}->db->prepare($sql);
    $st->execute( $uid, "junkword", 10, 10, $now,
        'junkword.test@example.com', "fakehash", 'fakesender@example.org' );

    my $sql = "INSERT INTO Emails
       (UserID,Word,InitialCount,Count,TimeAdded,Address,PrivateHash,Sender)
       VALUES (?,?,?,?,?,?,?,?);";
    my $st = $self->{'config'}->db->prepare($sql);
    $st->execute( $uid, "exceeded", 10, 0, $now, 'exceeded.test@example.net',
        "fakehash", 'fakesender@example.org' );
