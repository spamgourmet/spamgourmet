...
sub new {
    ...
    if ( $params{'config'} ) {
        ...
        die
            'Util must be initialized with an instance of Mail::Spamgourmet::Config.pm';
    }
    return $self;
}
...
sub hasFeature {
    ...
    if ( $feature =~ /\w+/ )
    {    # if a string was passed in, get the number from config
        $feature = $self->{'config'}->getFeature($feature);
    }
    if (   $userFeatureSetting
        && $feature
        && $userFeatureSetting % $feature == 0 )
    {
        return 1;
    }
    else {
...
sub getEatenMessageLog {
    ...
    my $newLog
        = time()
        . $self->setToLength( 55, $from )
        . $self->setToLength( 20, $for );
    my $log = '';
    if ( length($oldLog) > ( 85 * ( $numberOfEatenMessagesToLog - 1 ) ) ) {
        $log
            = $newLog
            . substr( $oldLog, 0,
            ( 85 * ( $numberOfEatenMessagesToLog - 1 ) ) );
    }
    else {
        $log = $newLog . $oldLog;
    }
